...
1. metrics-server 定期轮询集群所有 Node，通过 kubelet HTTPS API（10250端口）获取资源指标；
2. kubelet 从节点 cgroup / container runtime 获取 Pod 和 Node 的 CPU、Memory 使用数据，并通过 Summary API 返回；
3. metrics-server 聚合各节点指标，并将最新采集结果缓存在内存中（默认不持久化，重启后缓存清空）；
4. metrics-server 通过 Kubernetes Aggregation API 注册 metrics.k8s.io 扩展接口；
5. kubectl top、HPA Controller 等组件通过 kube-apiserver 访问 metrics.k8s.io API，获取当前资源指标。
