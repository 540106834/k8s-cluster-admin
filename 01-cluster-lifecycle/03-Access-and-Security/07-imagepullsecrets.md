...
```bash
# 编辑命名空间默认sa
kubectl edit sa default -n prod-api
...
```bash
# 查看创建的镜像凭证Secret
kubectl get secret harbor-secret -n prod-api

# 查看secret详细配置
kubectl describe secret harbor-secret -n prod-api

# 删除过期镜像凭证
kubectl delete secret harbor-secret -n prod-api
